...
public:
    ...
    void addStandardDeck(vector<Card> &Deck, bool useIndex)
    {
        Card temp;
        for (int i = 0; i < 52; i++)
        {
    ...
    void draw(vector<Card> &Deck, vector<Card> &Hand, vector<Card> &Graveyard, int &value, bool &bust)
    {
        ...
        if (value > 21)
        {
            ...
            {
                if (!Hand[i].getMinValueUsed()) //if it's a multivalued card and the value hasn't changed
                {
                    if (Hand[i].getMinValue() != Hand[i].getValue())
                    {
